...
export interface Destination { id: number; name: string; province: string; cover: string; slogan: string; reason: string; bestMonths: number[]; days: number; budgetMin: number; budgetMax: number; styles: string[]; score: number; features: string[]; route: string[]; tips: string[]; attractions: Attraction[] }
